...
class iris_detection():
    ...
    def detect_pupil(self):
        ...
        _, thresh = cv2.threshold(self._img.copy(), 100, 255, cv2.THRESH_BINARY_INV)
        cv2.imshow("Result tresh", thresh)
        # Now find the contours and then find the pupil in the contours.
        contours, hi = cv2.findContours(thresh, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
        ...
        for cont in contours:
            for c in cont:
                #print(c[0])
                x,y = c[0]
                cv2.putText(self._img,'.',(x,y),cv2.FONT_HERSHEY_PLAIN,0.8,(255, 255, 255),1)
        '''for l in c:
